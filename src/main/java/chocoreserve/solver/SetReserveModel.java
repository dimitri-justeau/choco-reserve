...
import chocoreserve.solver.constraints.ISetReserveConstraintFactory;
 ...
 */
public class SetReserveModel implements IReserveModel, ISetReserveConstraintFactory, IFeatureFactory {
    ...
    private SetVar universe, core, buffer, out;
    ...
    public SetReserveModel(RegularSquareGrid grid) {
        this.grid = grid;
        ...
        // Set vars
        this.universe = model.setVar("universe", IntStream.range(0, nbCells).toArray());
        this.core = model.setVar("core", new int[] {}, IntStream.range(0, nbCells).filter(i -> !grid.isInBorder(i)).toArray());
//        this.core = model.setVar("core", new int[] {}, IntStream.range(0, nbCells).toArray());
        ...
        this.model.partition(new SetVar[] {this.core, this.buffer, this.out}, this.universe).post();
        // Nb CC
